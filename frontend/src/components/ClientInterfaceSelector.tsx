    ...
    <Box sx={{ mb: 3 }}>
      ...
      <FormControl fullWidth sx={{ mb: 2 }} disabled={!selectedClient}>
        ...
        >
          {interfaces.length === 0 ? (
            <MenuItem disabled value="">
              {selectedClient ? 'No interfaces available for this client' : 'Select a client first'}
            </MenuItem>
          ) : (
      ...
      {required && selectedClient && interfaces.length === 0 && (
        <Alert severity="warning">
      ...
      {required && selectedClient && interfaces.length > 0 && !selectedInterface && (
        <Alert severity="info">
